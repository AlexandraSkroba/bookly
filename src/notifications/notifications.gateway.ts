...
export class NotificationsGateway implements OnModuleInit {
  ...
  constructor(@Inject() private notificationsService: NotificationsService,
  ...
  async sendNotification(id: number, subject: string) {
    const notification = await this.notificationsRepository.findOne({ where: { id }, relations: ['users'] });
    const userIds = notification.users.map((user, _index) => { return user.id });
